Component({
  ...
  methods: {
    ...
    drawPercentage(ctx, percentage) {
      ...
      ctx.beginPath();
      ctx.fillStyle = this.data.fontColor || this.data.g_primary;
    ...
    drawProgressCircular(startPercentage, ctx, canvas) {
      ...
      gradient.addColorStop(0, that.data.progressColor || that.data.g_primary);
      ...
      function drawFrame(callback) {
        ...
        if (startPercentage >= percentage) {
          ...
          that.data.startPercentage = startPercentage;
